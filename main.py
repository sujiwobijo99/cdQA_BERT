...
# Loading Datasets
df = pd.read_csv('dataset.csv', sep=";", converters={'paragraph': literal_eval})
# print(df.head())
content = df['content']
topic = df['topic']
link = df['link']
...
def answer_question(question, datasets):
    ...
    for text in datasets:
        ...
        input_ids = tokenizer.encode(question, text_string, truncation= True)
    ...
    print("Link for Answer:", link[id_max])
...
answer_question(question, content)